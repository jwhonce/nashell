...
int main(int argc, char *argv[]) {
  ...
  if (!no_embeddings && cfg->embedding.type &&
    ...
    printf("Initializing embeddings: type=%s model=%s\n",
           cfg->embedding.type,
           cfg->embedding.model_path ? cfg->embedding.model_path : cfg->embedding.model ? cfg->embedding.model
                                                                                        : "(default)");
  ...
  for (int i = 0; i < results.count; i++) {
    ...
    if (e->key) {
      if (strncmp(e->key, "lesson:", 7) == 0)
        type = "LESSON";
      else if (strncmp(e->key, "strategy:", 9) == 0)
        type = "STRATEGY";
      else if (strncmp(e->key, "skill:", 6) == 0)
        type = "SKILL";
      else if (strncmp(e->key, "task:", 5) == 0)
        type = "TASK";
    }
  ...
  if (results.count > 0) {
    ...
    for (int i = 0; i < results.count; i++) {
      ...
      if (k && strncmp(k, "lesson:", 7) == 0)
        n_lesson++;
      else if (k && strncmp(k, "strategy:", 9) == 0)
        n_strategy++;
      else if (k && strncmp(k, "skill:", 6) == 0)
        n_skill++;
      else
        n_other++;
    }
