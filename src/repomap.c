...
static int is_c_function_def(const char *line, char *fname, int maxlen) {
  ...
  const char *end = p + 1;
  while (p >= line && is_ident_char((unsigned char)*p))
    p--;
  p++;
...
static void extract_c_tags(rm_state_t *st, int file_idx) {
  ...
  for (int i = 0; i < f->n_lines; i++) {
            ...
            if (cl[0] == '}') {
              ...
              if (s2) {
                ...
                while (pp > cl && (*pp == ' ' || *pp == '\t'))
                  pp--;
                const char *ee = pp + 1;
                while (pp > cl && is_ident_char((unsigned char)*pp))
                  pp--;
    ...
    if (line[0] != ' ' && line[0] != '\t' && line[0] != '/' && strchr(line, '(') && strchr(line, ';')) {
      char fname[RM_MAX_IDENT_LEN];
      ...
      if (paren) {
        ...
        if (len > 1 && len < RM_MAX_IDENT_LEN) {
          ...
          int is_kw = 0;
          for (int k = 0; kw[k]; k++) {
            if (len == (int)strlen(kw[k]) && strncmp(fname, kw[k], (size_t)len) == 0) {
              is_kw = 1;
              break;
            }
          }
    ...
    for (int r = 0; r < n_ranked && chars_used < max_chars; r++) {
      ...
      if (frs[fri].n_show < 64) {
        ...
        for (int k = 0; k < frs[fri].n_show; k++) {
          if (frs[fri].show_lines[k] == def_line) {
            dup = 1;
            break;
          }
        }
