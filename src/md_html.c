...
static int is_separator_row(const char *p) {
  ...
  while (*p && *p != '\n') {
    if (*p == '-' || *p == ':')
      ...
      return 0; /* non-separator character */
    p++;
  }
  return has_dash;
}
...
#define TBL_MAX_ROWS 128
        ...
        struct {
          int s;
          int n;
        } cells[TBL_MAX_ROWS][TBL_MAX_COLS];
        ...
        while (md[scan] && is_table_line(md, scan) && nrows < TBL_MAX_ROWS) {
          ...
          while (p < line_end && col < TBL_MAX_COLS) {
            int cs = p, ce = p;
            while (ce < line_end && md[ce] != '|')
              ce++;
            /* Trim whitespace */
            int ts = cs, te = ce;
            while (ts < te && (md[ts] == ' ' || md[ts] == '\t'))
              ts++;
            while (te > ts && (md[te - 1] == ' ' || md[te - 1] == '\t'))
              te--;
...
#undef TBL_MAX_ROWS
    ...
    if (md[i] == '*' && md[i + 1] != '*' && md[i + 1] != ' ' && md[i + 1] != '\0' && (i == 0 || is_word_boundary(md[i - 1]))) {
      /* Scan for closing * at a word boundary */
      int j = i + 1;
      while (md[j] && md[j] != '\n') {
        if (md[j] == '*' && md[j + 1] != '*' && is_word_boundary(md[j + 1])) {
          /* Found valid closing * */
          i++;
    ...
    if (md[i] == '_' && md[i + 1] != '_' && md[i + 1] != ' ' && md[i + 1] != '\0' && (i == 0 || is_word_boundary(md[i - 1]))) {
