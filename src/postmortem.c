...
static const char *cause_str(fail_terminal_cause_t c) {
  switch (c) {
    case FAIL_TOOL_ERROR:
      return "tool_error";
    case FAIL_STEP_LIMIT:
      return "step_limit";
    case FAIL_NULL_RESULT:
      return "null_result";
    case FAIL_CYCLING:
      return "cycling";
    case FAIL_EMPTY_RESULT:
      return "empty_result";
  }
  return "unknown";
}
...
static const char *mechanism_str(fail_mechanism_t m) {
  switch (m) {
    case FMECH_FILE_EDIT_MISMATCH:
      return "file_edit_mismatch";
    case FMECH_UNREAD_REF:
      return "unread_ref";
    case FMECH_SHELL_RETRY:
      return "shell_retry";
    case FMECH_CONTEXT_EVICTION:
      return "context_eviction";
    case FMECH_WRONG_TOOL:
      return "wrong_tool";
    case FMECH_HALLUCINATION:
      return "hallucination";
    case FMECH_SPEC_VIOLATION:
...
postmortem_report_t *postmortem_analyze(const char *nash_dir, int max_sessions) {
  ...
  if (wd) {
    ...
    while ((we = readdir(wd)) != NULL) {
      ...
      if (snprintf(ws_sessions, sizeof(ws_sessions), "%s/%s/sessions",
                   ws_base, we->d_name) >= (int)sizeof(ws_sessions))
        continue;
  ...
  report->avg_efficiency = report->n_trajectories > 0
                             ? sum_efficiency / (float)report->n_trajectories
                             : 0.0f;
  report->avg_waste_ratio = report->n_trajectories > 0
                              ? sum_waste / (float)report->n_trajectories
                              : 0.0f;
