...
static int cmd_todo_list_all(command_ctx_t *ctx) {
  ...
  /* Global todo */
  {
    ...
    snprintf(gpath, sizeof(gpath), "%s/todo.md", ctx->dirs->data_dir);
  ...
  /* Enumerate workspaces */
  {
    ...
    snprintf(ws_dir, sizeof(ws_dir), "%s/workspaces", ctx->dirs->data_dir);
    ...
    if (d) {
      ...
      while ((ent = readdir(d)) != NULL) {
        ...
        if (snprintf(tpath, sizeof(tpath), "%s/%s/todo.md",
                     ws_dir, ent->d_name) >= (int)sizeof(tpath))
          continue;
