...
static void prompt(const char *label, const char *def, char *buf, size_t bufsz) {
    ...
    if (read_line(buf, bufsz) != 0 || buf[0] == '\0') {
        if (def) snprintf(buf, bufsz, "%s", def);
        else buf[0] = '\0';
    }
}
...
static int write_config_toml(const char *nash_dir,
    ...
    for (int i = 0; i < n_providers; i++) {
        ...
        if (sp->api_key_env[0] && !sp->store_key_in_file)
            fprintf(f, "api_key_env = \"%s\"\n", sp->api_key_env);
        else if (sp->api_key_env[0])
            fprintf(f, "api_key_env = \"%s\"\n", sp->api_key_env);
        if (sp->project_id[0])
...
int setup_run(const char *nash_dir, int add_only) {
    (void)add_only;  /* TODO: implement add-only mode */
    ...
    int rc = -1;
    switch (choice) {
    case 1: rc = configure_local(&providers[0]); break;
    case 2: rc = configure_anthropic(&providers[0]); break;
    case 3: rc = configure_openai(&providers[0]); break;
    case 4: rc = configure_vertex(&providers[0]); break;
    case 5: rc = configure_custom(&providers[0]); break;
    }
    if (rc != 0) {
    ...
    while (n_providers < 8 && prompt_yn("\nAdd another provider?", 0)) {
        ...
        rc = -1;
        switch (choice) {
        case 1: rc = configure_local(&providers[n_providers]); break;
        case 2: rc = configure_anthropic(&providers[n_providers]); break;
        case 3: rc = configure_openai(&providers[n_providers]); break;
        case 4: rc = configure_vertex(&providers[n_providers]); break;
        case 5: rc = configure_custom(&providers[n_providers]); break;
        }
        if (rc == 0) n_providers++;
    }
    ...
    for (int i = 0; i < n_providers; i++) {
        if (providers[i].api_key_value[0] && !providers[i].store_key_in_file) {
            ...
            fprintf(stderr, "  export %s=\"%s\"\n",
                    providers[i].api_key_env, providers[i].api_key_value);
        }
    }
